...
public class ProfessorEvaluationServiceImplTest {
    ...
    void 과제_채점_내역_조회() {
        ...
        // when
        EvaluationResponseDto responseDto = professorEvaluationService.get(professor.getId(), course.getId(), assignment.getId(), student.getId());

        // then
        assertThat(responseDto.id()).isEqualTo(evaluationResponseDto.id());
        assertThat(responseDto.score()).isEqualTo(evaluationResponseDto.score());
        assertThat(responseDto.feedback()).isEqualTo(evaluationResponseDto.feedback());

        assertThat(evaluationRepository.findById(responseDto.id()))
                .isPresent()
                .get()
                .satisfies(evaluation -> {
                    assertThat(evaluation.getSubmission()).isEqualTo(submissionRepository.findById(draftSubmission.getId()).get());
